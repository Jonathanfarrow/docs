...
description: "A deep dive into intent-driven agent architecture — from theory to a fully working chat agent with memory, strategies, and knowledge extraction."
---
...
This guide walks you through the architecture behind intent-driven agents and builds a complete, production-ready chat agent using `minns-sdk` and EventGraphDB.
...
With an agent backed by EventGraphDB, every interaction is stored as an event. The agent forms memories, extracts claims ("User likes Sci-Fi"), and learns strategies ("When booking fails, offer alternative showtimes"). By Friday, the agent *knows* the user, *remembers* the failure, and *has a strategy* for recovery.
...
Before every LLM call, the agent gathers context from EventGraphDB. This is the **Perceive** phase.
...
<Info>
  The three retrievals run in **parallel** with `Promise.all`. This keeps latency low — you're adding ~1 round-trip to EventGraphDB, not 3 sequential ones.
</Info>
...
The **Remember** phase. Every turn gets logged to EventGraphDB — the user message, the agent's reasoning, the action, and the outcome. This is what powers memory formation, claim extraction, and strategy learning.
...
async function runAgent(userId: string) {
  ...
  try {
    while (true) {
      ...
      // Gather context from EventGraphDB (parallel retrieval)
...
When you log a Context event with `enable_semantic: true` (the default for the SDK), EventGraphDB extracts atomic facts:
...
Because every event carries the same `goal("book_movie", ...)`, EventGraphDB groups them into an **episode**. When `goalProgress` hits `1.0`, the episode completes and becomes a candidate for long-term memory.
...
After several successful booking episodes, EventGraphDB detects a pattern:
...
The beauty of EventGraphDB is that trimmed messages aren't lost — they're already stored as events. Claims extracted from early turns persist in semantic memory even after the conversation window moves forward.
...
| **Event builder** | Logs every turn to EventGraphDB | `client.event().action().outcome().send()` |
...
The intent model gives your agent a clean separation between *thinking* (LLM) and *doing* (tools). EventGraphDB gives it the ability to *remember* and *improve*. Together, they turn a stateless chatbot into an agent that gets better with every conversation.
