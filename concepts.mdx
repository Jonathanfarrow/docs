---
title: "Core concepts"
description: "Understand the building blocks of Minns Memory Layer — Events, Episodes, Goals, Memory, Claims, and Strategies."
---
...
An **Event** is an atomic interaction — a user message, an AI action, a sensor reading, or a reasoning step. Every piece of data enters Minns Memory Layer as an event.
...
An **Episode** is a sequence of events forming a coherent task. Minns Memory Layer automatically groups events that share the same active goal into episodes.
...
A **Memory** is the long-term record of a significant episode. Once an episode completes, Minns Memory Layer consolidates it into a memory that can be recalled later by matching the current context against past contexts.
...
A **Strategy** is a learned behavioral pattern extracted from successful episodes. If an agent repeatedly follows the same sequence of actions to achieve a goal, Minns Memory Layer distills that pattern into a reusable strategy.
